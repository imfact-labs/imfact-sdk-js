...
// ── DID ─────────────────────────────────────────────────────────────────
import { RegisterModelFact as DIDRegisterModelFact } from "../operation/did/register-model"
import { CreateFact as DIDCreateFact } from "../operation/did/create-did"
import { UpdateDocumentFact } from "../operation/did/update-did-document"
import { AsymKeyAuth, LinkedAuth, Document, Service } from "../operation/did/document"
import { AllowedOperation } from "../operation/base"
...
function authFromJson(json: any): AsymKeyAuth | LinkedAuth {
    if (json.type === "LinkedVerificationMethod") {
        return new LinkedAuth(
            json.id,
            json.controller,
            json.targetId,
            (json.allowed as any[]).map(a => new AllowedOperation(a.operation, a.contract)),
        )
    }
...
function documentFromJson(json: any): Document {
    return new Document(
        json["@context"],
        json.id,
        (json.authentication as any[]).map(authFromJson),
        (json.verificationMethod as any[]).map(authFromJson),
        json.service && json.service.length > 0
            ? (json.service as any[]).map(s => new Service(s.id, s.type, s.service_end_point))
            : undefined,
    )
}
...
export function factFromJson(factJson: any): Fact {
    ...
    if (hint.includes(HINT.DID.REGISTER_MODEL.FACT)) {
        return new DIDRegisterModelFact(
            token,
            factJson.sender,
            factJson.contract,
            factJson.didMethod,
    ...
    if (hint.includes(HINT.DID.UPDATE_DID_DOCUMENT.FACT)) {
        return new UpdateDocumentFact(
            token,
            factJson.sender,
            factJson.contract,
            factJson.did,
            documentFromJson(factJson.document),
            factJson.currency,
        )
    }
